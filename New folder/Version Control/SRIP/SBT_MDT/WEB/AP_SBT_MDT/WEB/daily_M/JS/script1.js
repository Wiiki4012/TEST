          ...
          if (dataList && dataList.length > 0) {
            $.each(dataList, function (i, item) {
              ...
              if (index > -1) {
                ...
                if(charts[index].UPDATE_FREQ == 'week' && item.VALUE > 0) {
                  charts[index].date.push(moment(item.DATE, "MM/DD/YYYY").format("DD MMM"));
                  charts[index].value.push(item.VALUE);
                  const stepvaluetrigger = parseFloat(charts[index].LOWER_LIMIT)/4;
                  const stepvaluetarget = parseFloat(charts[index].UPPER_LIMIT)/4;
                  charts[index].LOWER.push(stepvaluetrigger);
                  charts[index].UPPER.push(stepvaluetarget);
                }
                else if(charts[index].UPDATE_FREQ == 'fortnight' && item.VALUE > 0) {
                  charts[index].date.push(
                    moment(item.DATE, "MM/DD/YYYY").format("DD MMM")
                  );
                  charts[index].value.push(item.VALUE);
                  const stepvaluetrigger = parseFloat(charts[index].LOWER_LIMIT)/2;
                  const stepvaluetarget = parseFloat(charts[index].UPPER_LIMIT)/2;
                  charts[index].LOWER.push(stepvaluetrigger);
                  charts[index].UPPER.push(stepvaluetarget);
                }
                else {
