...
function tasksData() {
  ...
  if (dataC) {
    ...
    const dataC =
      _kpi && _creationDate && _Kpi_hit_on && _submissiondate && preparedby;
    const err = !_kpi
      ? "Name of KPI"
      : !_creationDate
      ? "Creation Date"
...
function modal1open(data) {
  ...
  var subdate = data && data.DATE_SUBMISSION ? data.DATE_SUBMISSION : moment();
