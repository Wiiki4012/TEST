    ...
    ajaxCall(
        ...
        function(result) {
            ...
            if (dataList && dataList.length > 0) {
                ...
                document.getElementById("_staroftheday").src = "/XMII/CM/AP_SBT_MDT/V3/main_dashboard/image/placeholder_starofday.png";
                // console.log("no data");
            }
        },
        function(err) {
...
function escalationcheck(team) {
   // document.getElementById('_content5').innerHTML = "";
    ...
    if (split[0] == 'MDT ') {
                ...
                if (dataList && dataList.length > 0) {
                    ...
                    $.each(dataList, function(i, item) {
                        ...
                        htm +=
                            ...
                            "</td><td>" +
                            "<input type='text' style='text-align:center;' id='escComments" + item.ESCID + "' value='" + item.COMMENTS + "'>" +
                            "</td>" +
                            "<td class='hyperlink' onclick='updateEscStatus(" +
                            JSON.stringify(item) +
                            ")'>" +
                            "CLOSE" +
                            "</td></tr>";
                     //   $("#_content5").append(item.TEAM_NAME + "<br>");
                        // $("#content5").val.append(item.TEAM_NAME+"<br>");
                    });

                    // $("#_content5").html(escteam.join("\n"));
                    $("#content5").val(escteam.join());
        ...
        // let node = document.getElementById(_content5);
        // if (node.parentNode) {
        //     node.parentNode.removeChild(node);
        // }
        console.log("IIIIIIIIIIIIIIIINNNNNNNNNNNNNNNNNNNNNSSSSSSSSSSSSSSSIIIIIIIIIIIIIIDDDDDDDDDDDDDDEEEEEEEEEEEEE ESLE IF" + split[0]);
        const API =
            "/XMII/Illuminator?QueryTemplate=Test%2FQRY%2FSELECT_SBT_ESCALATIONS";



        const API_URL = ipAddress + API + contentType + externalCred;
        const Local_URL = "./JS/dummy_data/imgSelect.json";
        console.log("tttttttttttttttteeeeeeeeeeeeeeeeaaaaaaaaaaaaaaaaaaaaammmmmmmm" + team);
        const params = {
            'Param.1': team
        };
        console.log('k-', API_URL)
            ...
            function(result) {
                ...
                if (dataList && dataList.length > 0) {
                    ...
                    $.each(dataList, function(i, item) {
                        ...
                        console.log(item.ESCALATION + "---------------" + item.STATUS_OF_CLOSURE);

                     //   $("#_content5").html(item.ESCALATION + "&nbsp; <p style = 'color:red;font-weight: bold;'>" + item.STATUS_OF_CLOSURE + "</p><br>");

                      //  $("#content5").append(item.ESCALATION + "&nbsp; <p style = 'color:red'>" + item.STATUS_OF_CLOSURE + "</p><br>");
...
function updateEscStatus(item) {
    console.log("BBBBBBBBBBBBBYYYYYYYYYYYYYYYYYYYYYEEEEEEEEEEEEEEEEE" + item.ESCID);
    const commentid = "#escComments" + item.ESCID;
    const _comments = $(commentid).val();
    if (_comments != '---') {
        const API =
            "/XMII/Illuminator?QueryTemplate=Test%2FQRY%2FUpdate_Esclation_Status";
        ...
        ajaxCall(
            ...
            params,
            function(result) {
                escalationcheck(item.MDT_TeamName);
...
function attendenceMemberwithoutSNO(data, status) {
    ...
    $.each(data, function(i, item) {
        if (item.TEAM_MEMBER) {
            console.log(item)
            attid = item.TEAM_MEMBER;
            attid = attid.replace(/\s+/g, "");
            $("#attenList").append(
                `<div class="col s6 m6 radioList">
          <h5 class="_name" title="${item.TEAM_MEMBER}">${item.TEAM_MEMBER}</h5>
          <label class="_radio">
...
function starload() {
    ...
    const team = split[1];
    console.log("-----------------tttttttttttttteeeeeeeeeeeaaaaaaaaaam" + team);
    starUpload(teamName);
}
