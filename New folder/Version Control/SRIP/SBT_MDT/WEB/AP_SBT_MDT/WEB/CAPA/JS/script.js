...
function delete5why(data) {
    ...
    ajaxCall(
        ...
        function(result) {
            ...
            $("#_5why tbody").append('<td colspan="6">No Data</td>');
        },
        function(err) {
            console.log("err", err);
        }
...
function tasksData() {
  ...
  //  alert(selected);
    ...
    const dataC = _kpi && _creationDate && _Kpi_hit_on && _submissiondate;
    if (dataC) {
        if (yetname != 'Yet to Assign' || yetname != '') {
...
function modal1open(data) {
    ...
    var subdate = data && data.DATE_SUBMISSION ? data.DATE_SUBMISSION : moment().add('10', 'days');
    ...
    ajaxCall(
        ...
        function(result) {
            ...
            if (dataList && dataList.length > 0) {
                ...
                $.each(dataList, function(i, item) {
                    if (item.STATUS == 'Closed') {
                        tdata += `<tr><td colspan="3">${item.KPI}</td>
                <td>${item.WHAT}</td>
...
function changeColor() {
    let color = grey;
    var optionElement = document.getElementById('_participants');
