...
function addcapa() {
  ...
  if(kpi != '' || CAPA != '' || capawhat != '' || capawho != '' || capawhen != '' || capastatus != '') {
    const API =
    "/XMII/Illuminator?QueryTemplate=SBT_MDT%2FQuery%2FSELECT_5WHYs";
    const API_URL = ipAddress + API + contentType + externalCred;
...
function executeStatus(team) {
  ...
  ajaxCall(
    ...
    function (result) {
      ...
      const dataList = dataC && dataC.length > 0;
      const data1 =
        dataList && dataC[0].openMTD && dataC[0].openMTD != "N/A" ? dataC[0].openMTD : 0;
      const data2 =
      ...
      const data3 =
        dataList && dataC[0].TOTALMTD && dataC[0].TOTALMTD != "N/A"
          ...
          : 0;
      const data4 =
        dataList && dataC[0].AVGDELAY && dataC[0].AVGDELAY != "N/A" ? dataC[0].AVGDELAY : 0;
      const data5 =
        dataList && dataC[0].overallopen && dataC[0].overallopen != "N/A" ? dataC[0].overallopen : 0;
      const data6 =
        dataList && dataC[0].ClosedYTD && dataC[0].ClosedYTD != "N/A" ? dataC[0].ClosedYTD : 0;
      const data7 =
        dataList && dataC[0].totalYTD && dataC[0].totalYTD != "N/A" ? dataC[0].totalYTD : 0;
      const data8 =
        dataList && dataC[0].AvgYTD && dataC[0].AvgYTD != "N/A" ? dataC[0].AvgYTD : 0;
      $("#_PSOMTD").text(data1);
      $("#_PSCMTD").text(data2);
      $("#_TPSMTD").text(data3);
      $("#_ADCYTD").text(data4);
    ...
    function (result) {
      ...
      if (dataList && dataList.length > 0) {
       ...
       //console.log("cur", currentdate);
        ...
        console.log("cur", currentdate);
        const _filterOverdue = dataList.filter(x=>new Date(x.DATE_SUBMISSION) < currentdate && x.STATUS !== 'Closed');
        console.log("overdue", _filterOverdue);
        const _filterClosed = dataList.filter(x=>x.STATUS === 'Closed');
        console.log("closed", _filterOverdue);
        const _filter = dataList.filter(x=>x.STATUS !== 'Closed' && new Date(x.DATE_SUBMISSION) >= currentdate);       
        ...
        $.each(datalists, function (i, item) {
         ...
         console.log("close", closuredate);
          var overdue =  ((closuredate < currentdate) && (item.STATUS != "Closed")) ? "overdue" : "";
          htm +=
            "<tr class='"+overdue+"'><td colspan='2'>" +
            item.KPI +
            "</td><td>" +
            item.PROBLEM +
            "</td><td>" +
            ...
            item.OTHERS +
	          "</td><td>" +
            ((item.ASSESSMENT_STATUS != "Approved" && item.STATUS == "Closed") ? "<button id='assessment_"+item.CAPAID+"' class='"+(item.ASSESSMENT_STATUS != 'Approved' ? 'waves-effect waves-light btn red asses' : 'waves-effect waves-light btn asses')+"' onclick='assessmentmodel("+item.CAPAID+")'>"+ (item.ASSESSMENT_STATUS != 'Approved' ? 'To be Modified' : 'Approved') +"</button>" : (item.ASSESSMENT_STATUS == "Approved" ? "Approved by FI ITF" : "Problem Solving is Open!")) +
            "</td>" +
            "<td class="+"action"+item.STATUS+"><span>---</span><span class="+"action"+item.STATUS+" s"+" id='edit" +
          "'onclick='modal1open(" +
          JSON.stringify(item) +
          ")'>" +
          "Edit" +
          "</span></td><td class="+"action"+item.STATUS+"><span>---</span><span class="+"action"+item.STATUS+" id='delete" +
          "'onclick='deleteTasks(" +
          JSON.stringify(item) +
          ")'>" +
          "Close" +
...
function tasksUpdate(
  ...
  const API =
    "/XMII/Illuminator?QueryTemplate=SBT_MDT%2FQuery%2FUPDATE_CAPA";
  const API_URL = ipAddress + API + contentType + externalCred;
  const Local_URL = "./JS/dummy_data/NEW_SKU_DETAILS.json";
...
function modal1open(data) {
  ...
  var KPIHITDATE =
    data && data.KPIHITON ? data.KPIHITON : moment();
  ...
  var subdate =
    data && data.DATE_SUBMISSION ? data.DATE_SUBMISSION : moment();
...
function closetasksData() {
  ...
  if(preparedby !== '') {
    const API =
    "/XMII/Illuminator?QueryTemplate=SBT_MDT%2FQuery%2FCLOSE_CAPA";
  const API_URL = ipAddress + API + contentType + externalCred;
  const Local_URL = "./JS/dummy_data/NEW_SKU_DETAILS.json";
